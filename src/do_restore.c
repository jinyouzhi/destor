	...
	while ((c = sync_queue_pop(restore_chunk_queue))) {
		...
		if (CHECK_CHUNK(c, CHUNK_FILE_START)) {
			...
			if (destor.simulation_level == SIMULATION_NO||destor.simulation_level == SIMULATION_FSL_DATA) {
				assert(fp == NULL);
				filepath = sdscat(filepath, ".0000");
				int i=0;
				int k=sdslen(filepath);
				while (access(filepath, 0) == 0) {//this file already exists
					filepath[k-3]=i/100+'0', filepath[k-2]=i%100/10+'0', filepath[k-1]=i%10+'0';
					//filepath = sdscat(filepath, last);
					//NOTICE("try file name:%s", filepath);
					i++;
				}
			...
			assert(destor.simulation_level == SIMULATION_NO||destor.simulation_level == SIMULATION_FSL_DATA);
...
void do_restore(int revision, char *path) {
	...
	restore_chunk_queue = sync_queue_new(destor.index_segment_algorithm[1]*2);
	restore_recipe_queue = sync_queue_new(destor.index_segment_algorithm[1]*2);
